...
public class ButtonControl extends AbstractControl {
    ...
    public void constructDOM(Element e, Document doc, Element result) {
        Element div = doc.createElement("div");
        // FIXME shouldn't hardcode the style (especially the color)
        div.setAttribute("style", "border: 1px outset black;");
        result.appendChild(div);
        if(e.getNodeName().equalsIgnoreCase("input")) {
            div.appendChild(doc.createTextNode(e.getAttribute("value")));
        } else {
            NodeList children = e.getChildNodes();
            for (int i = 0; i < children.getLength(); i++) {
                div.appendChild(children.item(i).cloneNode(true));
            }
        }
    }
}
