...
public class SWTFontResolver implements FontResolver {
    ...
    private static final String[] _defaults_sans = { "sans-serif", "sans",
            "sansserif", "helvetica", "arial" };
    private static final String[] _defaults_serif = { "serif", "Serif",
            "times new roman", "times" };
    private static final String[] _defaults_monospace = { "monospace",
    ...
    private void init() {
        ...
        // system fonts
        String system_font_family = _device.getSystemFont().getFontData()[0]
            .getName();
        _default_fonts = new HashMap();

        for (int i = 0; i < _defaults_sans.length; i++) {
            if (_device.getFontList(_defaults_sans[i], true).length > 0) {
                _default_fonts.put("sans-serif", _defaults_sans[i]);
                break;
            }
        }
        if (_default_fonts.get("sans-serif") == null) {
        ...
        _system_font = new SWTFSFont(systemFont, systemFont.getFontData()[0]
            .getHeight(), true);
    }
    ...
    public FSFont resolveFont(SharedContext renderingContext,
        ...
        if (spec.families != null) {
            for (int i = 0; i < spec.families.length; i++) {
                FSFont font = resolveFont(renderingContext, spec.families[i],
                    spec.size, spec.fontWeight, spec.fontStyle, spec.variant);
                if (font != null) {
                    return font;
                }
            }
        ...
        FSFont font = resolveFont(renderingContext, "sans-serif", spec.size,
            spec.fontWeight, spec.fontStyle, spec.variant);
        if (font != null) {
            return font;
        }

        XRLog.cascade(Level.WARNING, "Falling back to default system font. "
                + spec.toString());
    ...
    private SWTFSFont resolveFont(SharedContext ctx, String font, float size,
        ...
        String font_instance_name = getFontInstanceHashName(font, nSize,
        ...
        if (fd.length > 0) {
            ...
            if (style != null
                    && (style == IdentValue.ITALIC || style == IdentValue.OBLIQUE)) {
                style_bits |= SWT.ITALIC;
            }

            SWTFSFont fnt = new SWTFSFont(new Font(_device, fd[0].getName(),
                nSize, style_bits), size2D);
    ...
    private static String getFontInstanceHashName(String name, int size,
            IdentValue weight, IdentValue style, IdentValue variant) {
